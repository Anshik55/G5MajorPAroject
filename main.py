...
# test images
X_test = np.zeros((len(test_ids), IMG_HEIGHT, IMG_WIDTH, IMG_CHANNELS), dtype=np.uint8)   #y_test
sizes_test = []
...
preds_val = model.predict(X_train[int(X_train.shape[0]*0.9):], verbose=1)        # y_pred
preds_test = model.predict(X_test, verbose=1)   

iou_value=0.8

# def evaluation(X_train,preds_val,iou_value):
  
#     truth=np.squeeze(X_train)
#     preds=np.squeeze(preds_val)
...
# evaluation(X_test,preds_test,iou_value)

# print("pred_test",preds_test)
